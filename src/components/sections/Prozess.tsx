...
export function Prozess() {
  const { t } = useI18n();
  return (
    <section id="prozess" className="bg-bg-soft py-14 sm:py-16 lg:py-24">
      <div className="mx-auto w-full max-w-7xl px-5 md:px-8">
        ...
        <p className="mt-3 text-sm text-white/70 sm:text-base">{t.prozess.text}</p>

        <ol className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6 lg:mt-12 lg:grid-cols-4">
          {t.prozess.steps.map((s) => (
            <li
              key={s.n}
              className="rounded-lg border border-white/10 bg-bg p-5 sm:p-6"
            >
